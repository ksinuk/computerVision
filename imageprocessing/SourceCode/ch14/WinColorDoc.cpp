...
void CWinColorDoc::m_ImgSobel(unsigned char *InImg, unsigned char *OutImg, int height, int width)
{
	for(int i=1; i<height-1; i++)
	{
		for(int j=1; j<width-1; j++)
		{
			int xedg = -InImg[(i-1)*width+j-1]+InImg[(i-1)*width+j+1]
					 -2*InImg[i*width+j-1]  +2*InImg[i*width+j+1]
					   -InImg[(i+1)*width+j-1]+InImg[(i+1)*width+j+1];
			int yedg = -InImg[(i-1)*width+j-1]-2*InImg[(i-1)*width+j]-InImg[(i-1)*width+j+1]
					   +InImg[(i+1)*width+j-1]+2*InImg[(i+1)*width+j]+InImg[(i+1)*width+j+1];

			int mag = (int)sqrt(xedg*xedg+yedg*yedg);
			CLIP(mag,0,255);
			OutImg[i*width+j] = (unsigned char)mag;
		}
	}
