...
void CWinTestDoc::m_MorphologyGrayDilation(int height, int width)
{
	int r,c,i;
	int mx,my;
	int min,max,sum,where;
	int *pTmpImg;
	float constVal1,constVal2,newValue;
	...
	for(r=0;r<height;r++)
		for(c=0;c<width;c++)
		{
			...
			for(i=1;i<m_pSEGray[0].row;i++)//pSE[0].row: Structure Element의 크기
			{
				...
				if(mx>=0 && mx<width && my>=0 && my<height)
				{
					sum=m_InImg[my][mx]+m_pSEGray[i].grayval;
					if(sum>max)
						max=sum;
				}
			}
