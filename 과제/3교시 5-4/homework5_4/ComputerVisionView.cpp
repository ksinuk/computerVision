...
void CComputerVisionView::OnDraw(CDC* pDC)
{
	...
	CImage input_image,input_image_1, output_image;
	if (!pDoc->GetInputMFCImage(input_image)) input_image.Draw(pDC->GetSafeHdc(), 0, 0, input_image.GetWidth(), input_image.GetHeight());
	if (!pDoc->GetInput1MFCImage(input_image_1)) input_image_1.Draw(pDC->GetSafeHdc(), input_image_1.GetWidth()+10, 0, input_image_1.GetWidth(), input_image_1.GetHeight());
	if (!pDoc->GetOutputMFCImage(output_image)) output_image.Draw(pDC->GetSafeHdc(), 2*output_image.GetWidth() + 20, 0, output_image.GetWidth(), output_image.GetHeight());
}
